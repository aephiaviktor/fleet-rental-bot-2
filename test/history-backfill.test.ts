...
test('acceptance events recover exact timestamps and rate only for matching confirmed rental',()=>{
 ...
 const row=decodeAcceptedRentals(tx,'sig','profile')[0];assert.equal(row.rate,1);assert.equal(row.rentTotal,2);assert.equal(row.startEstimated,false);assert.equal(row.bid,0);
...
test('backfill also scans observed rental accounts to recover accept signatures missed by borrower PDA',async()=>{
 ...
 db.prepare('INSERT INTO rental_history VALUES (?,?,?,?)').run('profile','account7',100000,JSON.stringify({id:'account7',contract:'account6',borrower:'account0',start:100000,end:160000,rate:2,rentTotal:2/1440,bid:0,currency:'None',observedAt:150000}));db.exec("CREATE TABLE history_cursor(profile TEXT,wallet TEXT,signature TEXT,PRIMARY KEY(profile,wallet))");db.prepare('INSERT INTO history_cursor VALUES (?,?,?)').run('profile','rental:account7','sig');db.close();
...
test('verified close event attaches earned points only to the matching completed cycle',async()=>{
 ...
 const event={type:'RentalClosed',rentalState:'r',contract:'c',borrower:'b',pointsAwarded:5000000000n};
...
test('closed-rental rewards persist through backfill and subsequent account reads',async()=>{
 ...
 const idl=createRequire(import.meta.url)('@sly-rentals/core/idl');const data=Buffer.concat([Buffer.from(idl.events.find((e:any)=>e.name==='RentalClosed').discriminator),Buffer.from(core.getRentalClosedEncoder().encode({rentalState:zero,contract:zero,borrower:zero,ownerEarned:0n,feeCollected:0n,pointsAwarded:5000000000n}))]);
